...
Registered research tasks currently include `security_identity`, `market_trend`, `etf_market`, `security_valuation`, `valuation_compare`, `research_content`, `capital_events`, and `market_signals`. `market_trend` never silently mixes adjustment bases; valuation comparisons never mix dates or metric definitions; research materials, capital events, and market signals never erase their source role, time boundary, unit, direction, provenance, or coverage.
