...
description: Produce auditable A-share and SSE ETF research with a deterministic CLI for canonical identity, OHLCV trends, ETF quotes, valuation, reports, disclosures, capital flows, dragon-tiger records, lockups, margin data, block trades, shareholder counts, dividends, and caller-provided evidence. Use when a user needs an explicit as-of date, provenance, calculation lineage, conflicts, and limitations. Do not use for recommendations, price targets, trading, or unsupported live-data claims.
---
...
3. Treat a name, abbreviation, or bare code as a security clue. Use `security_identity` as a standalone preflight when the user is asking which security they mean. Subject-based research tasks, including `research_content` and `capital_events`, accept the clue and perform the same fail-closed resolution internally; use only the canonical subject returned in their result. Ask the user to choose when resolution requires clarification, and never guess the exchange.
...
Registered research tasks currently include `security_identity`, `market_trend`, `etf_market`, `security_valuation`, `valuation_compare`, `research_content`, and `capital_events`. `market_trend` never silently mixes adjustment bases; valuation comparisons never mix dates or metric definitions; research materials and capital events never erase their source role, time boundary, unit, or direction.
