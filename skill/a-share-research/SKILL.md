...
4. Choose one research path:
   ...
   - For a packaged research flow, run `research_workflow` with top-level `window: null`, `parameters.workflow` containing one documented `id` and `version: "1.0"`, and workflow-specific values under `parameters.inputs`. Choose `single_security_valuation`, `valuation_comparison`, `theme_report_research`, or `new_security_research`; never submit caller-defined steps or a DAG. The first three wrap one existing ResearchTask. `new_security_research` runs identity, institutional coverage, valuation, provider board membership, fund flow, dragon-tiger, lockup, and margin-trading steps with explicit report, market, and lockup windows. Preserve every step's ResearchTask and result. Identity blockage skips dependent steps; another blocked step does not stop independent steps, and the overall status remains `limited` or `blocked` as reported.
...
Registered research tasks currently include `security_identity`, `market_trend`, `etf_market`, `etf_options`, `security_valuation`, `valuation_compare`, `research_content`, `capital_events`, `market_signals`, and `research_workflow`. `market_trend` never silently mixes adjustment bases; ETF options never merge `M` / `A` series or hide tied ATM strikes; workflows never bypass a leaf task's evidence or status; valuation comparisons never mix dates or metric definitions; research materials, capital events, and market signals never erase their source role, time boundary, unit, direction, provenance, or coverage.
...
- For research workflows, present the requested and executed workflow versions, fixed plan, brief, and every step's state, ResearchTask, result, or skip reason. Keep top-level evidence and diagnostics associated with their `step_id`; never hide a blocked step behind a more favorable overall summary.
...
- Never accept caller-defined workflow steps, recursively invoke `research_workflow`, loosen `source_policy` for a leaf task, or promote a leaf result's status.
