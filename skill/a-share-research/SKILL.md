...
description: Research A shares and SSE ETFs through an evidence-first deterministic CLI. Use for canonical security identity, prices and trends, ETF or ETF-option quotes, valuation and comparisons, research materials including F10, capital events, market signals, fixed research workflows, or caller-provided evidence when explicit dates, provenance, calculation lineage, conflicts, coverage, limitations, and evidence-backed interpretation matter.
---
...
2. Select the matching `ResearchTask` from [references/cli-contract.md](references/cli-contract.md). Use `security_identity` for identity-only questions; route prices and trends, ETF or ETF options, valuation, research materials, capital events, market signals, and packaged workflows only to their documented task types. Use compatibility commands only for the cases documented there. Completion: `task_type`, subject scope, window, parameters, and source policy match the question exactly.
...
5. Confirm the returned `task_type` and scope answer the user's question. After `valuation`, confirm its `research.question` matches the requested capability. Do not expect `research.question` from `resolve`, `close`, or `validate-bundle`. If the result is narrower, present only what the CLI formed. Completion: no absent result field or wider capability has been inferred.
...
- Preserve source roles. Cross-source agreement may expose consistency but does not by itself make an experimental or caller-provided observation source-verified.
